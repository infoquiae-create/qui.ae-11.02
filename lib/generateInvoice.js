...
import logoSrc from "../assets/logo/Qui Logo G (1).png";
...
// Use Noto Sans Arabic by default for Arabic script support; override via env if needed
const UNICODE_FONT_NAME = 'NotoSansArabicPDF';
const UNICODE_FONT_REG_VFS = 'NotoSansArabic-Regular.ttf';
const UNICODE_FONT_BOLD_VFS = 'NotoSansArabic-Bold.ttf';
...
async function renderArabicAsImage(text, fontSize = 12) {
    // Use canvas to render Arabic text (avoids jsPDF's RTL issues)
...
async function ensureUnicodeFont(doc) {
    ...
    try {
        const [regRes, boldRes] = await Promise.all([
            fetch(UNICODE_FONT_REG_URL, { mode: 'cors' }),
            fetch(UNICODE_FONT_BOLD_URL, { mode: 'cors' })
        ]);
        
        if (!regRes.ok || !boldRes.ok) {
            throw new Error(`Font fetch failed: ${regRes.status}, ${boldRes.status}`);
        }
        ...
        if (typeof doc.setR2L === 'function') {
            doc.setR2L(true); // helps Arabic shaping in jsPDF 2.5+
        }
    } catch (e) {
        console.warn('Arabic font failed to load, using fallback:', e.message);
        // If font fails to load, fallback to core font (non-Latin text may not render)
    }
}

// helpers
const formatInr = (n) => `${CURRENCY_SYMBOL}${Number(n || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
...
export const generateInvoice = async (order) => {
    ...
    // Bill To (left) - use canvas for Arabic text
    ...
    const nameText = String(order?.address?.name || 'Customer');
    if (isArabicText(nameText)) {
        // Render Arabic name on canvas as image
        try {
            const nameImg = await renderArabicAsImage(nameText, 12);
            if (nameImg) {
                doc.addImage(nameImg, 'PNG', margin.left, y + 6, 60, 8);
                y += 3;
            }
        } catch (e) {
            doc.text(nameText, margin.left, y + 8);
        }
    } else {
        doc.text(nameText, margin.left, y + 8);
    }
    ...
    const streetText = String(order?.address?.street || '');
    if (isArabicText(streetText)) {
        try {
            const streetImg = await renderArabicAsImage(streetText, 10);
            if (streetImg) {
                doc.addImage(streetImg, 'PNG', margin.left, y + 12, 70, 6);
            }
        } catch (e) {
            doc.text(streetText, margin.left, y + 14);
        }
    } else {
        doc.text(streetText, margin.left, y + 14);
    }
    
    const cityText = `${order?.address?.city || ''}, ${order?.address?.state || ''} - ${order?.address?.zip || ''}`;
    doc.text(cityText, margin.left, y + 20);
    ...
    const computedTotal = Number(order?.total ?? (subtotal + shippingFee - discount));
    doc.text('TOTAL:', labelX, totalY);
    doc.text(formatInr(computedTotal), rightX, totalY, { align: 'right' });

    // Footer
    doc.setDrawColor(230);
    doc.line(margin.left, pageHeight - margin.bottom - 5, rightX, pageHeight - margin.bottom - 5);
    doc.setFont(UNICODE_FONT_NAME, 'normal'); doc.setFontSize(9); doc.setTextColor(120);
